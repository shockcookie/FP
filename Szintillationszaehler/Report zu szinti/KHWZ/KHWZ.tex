...
\begin{document}
	...
	For the analysis on the logarithmic scale, the $\ln$ of every count value was computed. The error was propagated with the Gaussian error propagation as seen in \ref{err_log}.
	\begin{equation}
		\Delta y = \Delta x \cdot \frac{\partial}{\partial x} y = \frac{\partial}{\partial x} \ln(x) = \frac{\Delta x}{x}
		\label{err_log}
	\end{equation}
	
	This data and its errors are illustrated in figure 
	\ref{data_log}.	
	
	\begin{figure}[h]
		\includegraphics[scale=0.6]{Bilder/data_log_corrected}
		\centering
		\caption[Data with on logarithmic scale]{\small Logarithmic scaled plot all data points.}
		\label{data_log}
	\end{figure} Following this, almost the same steps as in the analysis on the linear scale
	were carried out.
	Starting with a linear fit in the background data range, the background was compensated. The due to the high uncertainty of the slope from this fit(vgl. parameters shown in \ref{log_bkg}), only the offset of the fitted line was used for compensation.
	(fig. \ref{log_bkg}), the background was subtracted from the original logarithmic data (fig. \ref{log_clean}). 
	\begin{figure}[h]
		\includegraphics[scale=0.6]{Bilder/lin_bkgnd_corrected}
		\centering
		\caption[Logarithmic scaled Background]{\small Logarithmic scaled plot of the background data with the fitted
			line. The Parameters of the linear regression used to compute the parameter line and the fitted parameters are noted in the lower left corner of the figure.}
		\label{log_bkg}
	\end{figure}
		\begin{figure}[h]
		\includegraphics[scale=0.6]{Bilder/log_nobkg_corrected}
		\centering
		\caption[Data without Background]{\small Logarithmic scaled plot of all data with subtracted background.}
		\label{log_clean}
	\end{figure}

	
	The main difference of this part was the linear fit 
	...
	determined:	$T_{\frac{1}{2} \textrm{log}} = (139.2\pm2.8)\,ns$
		
	\begin{figure}[h]
		\includegraphics[scale=0.6]{Bilder/log_end_corrected}
		\centering
		\caption[Logarithmic Plot]{\small Logarithmic scaled plot of the region of interest and the fitted curve. The parameters of the fitted curve are noted in the upper left corner of the figure.}
	...
	\end{figure}
 	...
 	To decrease the difference between literature value and the computed one, the most effective method would be to increase the right bound of the data used for the exponential fit. This, viewed in the context of physics, can be	explained with the signal processing. The bipolar signal from the main amplifier can slightly shift, causing the single canal analyzer to trigger on inaccureate times. This results in a shifted position of the datapoint, smearing the theoretically 	sharp edges of the decay curve.\\
  	...
  	The main issue of the log scaled calculation is the alomost 1.5 times too great computed half life of the logarithmic plot. Some of it can be explained by choosing the wrong range of values to fit, but this can not be the only explanation. The error on this value is reasonable for the given data. The computed half life is still far from the literature value, resulting in a compatibility of  $15\,\sigma$. 
...
%	\end{figure}
	...
	\end{figure}
	\begin{figure}
		\includegraphics[scale=0.5]{Bilder/HighThresh}
		\centering
...
\includepdf[pages=1-,scale=0.9]{Bilder/KHWZ_notes}
